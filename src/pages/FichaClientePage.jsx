...
import {
  ...
  Wallet, History, Gift, Activity, PlusCircle, FileText,
} from 'lucide-react'
...
export default function FichaClientePage() {
  ...
  const { cliente, posicion, solicitudes = [], historial = [], oferta, indicadores } = ficha
  ...
  return (
    <>
      ...
      <Card title="Solicitudes de crédito en trámite" icon={FileText} style={{ marginTop: 16 }}>
        {solicitudes.length === 0 ? (
          <p className="hb-table-empty" style={{ margin: 0 }}>No hay solicitudes de crédito en trámite.</p>
        ) : (
          <div className="hb-table-wrap">
            <table className="hb-table">
              <thead>
                <tr>
                  <th>Expediente</th>
                  <th className="num">Monto solicitado</th>
              ...
              <tbody>
                {solicitudes.map((s, i) => (
                  <tr key={i}>
                    ...
                    <td className="num"><Money value={s.monto} /></td>
                    <td className="num">{s.plazo_meses ?? '—'} m</td>
