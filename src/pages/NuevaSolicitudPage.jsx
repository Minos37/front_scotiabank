...
import { consultarBuro } from '../services/evaluacionService.js'
...
export default function NuevaSolicitudPage() {
  ...
  const onSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      // 1. Validar lista de inhabilitados y buró (RV-11)
      const buro = await consultarBuro({ dni: f.numero_documento.trim() })
      if (buro.en_lista_negra || buro.calificacion_sbs === 'PERDIDA') {
        setError(`CLIENTE BLOQUEADO: El solicitante figura en la lista de inhabilitados o tiene calificación SBS PERDIDA. Motivo: ${buro.motivo_bloqueo || 'Riesgo de cumplimiento financiero elevado.'}`)
        setSaving(false)
        return
      }
        ...
        <Card title="Condiciones del crédito" icon={Coins} style={{ marginTop: 16 }}>
          <div className="hb-grid-3">
            ...
            <div className="hb-field">
              ...
              <select className="hb-select" value={f.seguro_desgravamen ? 'si' : 'no'}
                onChange={(e) => {
                  const tiene = e.target.value === 'si';
                  setF((s) => ({
                    ...s,
                    seguro_desgravamen: tiene,
                    tea_referencial: tiene ? '40.92' : '43.92'
                  }));
                }} required>
                <option value="si">Con Seguro (TEA 40.92%)</option>
                <option value="no">Sin Seguro (TEA 43.92%)</option>
              </select>
            </div>
            <div className="hb-field">
              <label>TEA referencial (%)</label>
              <input className="hb-input" value={f.tea_referencial} readOnly style={{ background: '#f5f5f5', cursor: 'not-allowed' }} />
            </div>
            <div className="hb-field">
              <label>Moneda</label>
